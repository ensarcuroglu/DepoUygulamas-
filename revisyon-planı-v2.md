...
| B3 | ~~Conftest'e ortak mock fixture'lari tasi: `_make_service()` tekrarini gider~~ ✅ TAMAMLANDI (2026-04-02) — tests/unit/services/conftest.py (3 fixture: palet_stok_service_mock, stok_cikis_service_mock, palet_sorgulama_service_mock) + tests/unit/use_cases/conftest.py (2 fixture: irsaliye_olustur_uc_mock, sevkiyat_guncelle_uc_mock). 5 test dosyasından _make_service()/_make_use_case() kaldırıldı; fixture (service, mocks_dict) tuple döndürür → mevcut test mantığı değişmedi. | DRY, bakim maliyeti |
| B4 | ~~Sinir degeri testleri ekle: koli_adedi=0, bos string, NULL FK, max uzunluk~~ ✅ TAMAMLANDI (2026-04-02) — tests/unit/dto/test_dto_sinir_degerleri.py (YENİ, DB gerektirmez): 37 fonksiyon → ~80 parametrize test case; UrunOlusturRequestDTO (isim whitespace/max, ic_adet=0, fiyat<0, min_stok<0, EAN 7/15 hane, birim geçersiz, marka_id=0), SiparisOlusturRequestDTO (boş kalemler, duplicate urun_id, miktar=0, birim_fiyat<0, kdv>100, geçmiş tarih), StokHareketiOlusturRequestDTO (miktar=0, cikis_no_aciklama, barkod 51 char), SevkiyatPlaniOlusturRequestDTO (siparis_id=0, geçersiz durum, plaka>20). API katmanı: test_urunler_api.py +8 test (isim boş/uzun, fiyat<0, ic_adet=0, birim geçersiz, ean<8, marka_id=0, barkod çakışma 409), test_siparisler_api.py +6 test (musteri_adi boş/uzun, kalemler boş, duplicate urun_id, miktar=0, birim_fiyat<0). | Uretimde surpriz hatalari onle |
| B5 | ~~Es zamanlilik testi ekle: ayni palet_no'ya paralel giris, FIFO cikis yarisi~~ ✅ TAMAMLANDI (2026-04-07) — tests/integration/test_eszamanlilik.py (7 test senaryosu): Çift palet girişi engeli (UNIQUE constraint), FIFO çıkış yarışı (SELECT FOR UPDATE kilidi), eşzamanlı kısmi çıkış ve deadlock dayanıklılığı threading ile MySQL test veritabanında doğrulandı. | FIFO ve palet islemleri icin kritik |
| B6 | Entity-ORM alan uyum testi ekle: entity alanlari ↔ ORM sutunlari otomatik karsilastirma | models.py + entity + mapper tutarliligi | ✅ TAMAMLANDI (2026-04-07)
