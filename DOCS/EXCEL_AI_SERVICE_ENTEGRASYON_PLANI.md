...
[ ] **5. LLM katmani:** `app/infrastructure/llm/ollama_client.py` (DocAi `OllamaTextClient` pattern'i) + `app/application/agents/pandas_qa_agent.py` (LangChain `create_pandas_dataframe_agent` + sistem promptu Turkce, `allow_dangerous_code=False`).
...
- **Guard'lar:** `allow_dangerous_code=False`; pandas agent'in `eval`/`exec`'e erismesi engellenir. Sutun/satir limitleri ve sayfa basina max hucre kontrolleri `excel_loader`'da.
