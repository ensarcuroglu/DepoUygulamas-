"""Health endpoint for AssistantAiService."""
...
async def healthz(settings: Settings = Depends(get_settings)) -> dict:
    ...
    if settings.assistant_llm_model not in names:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail=f"Ollama asistan modeli bulunamadi: {settings.assistant_llm_model}",
        )
    ...
    return {
        ...
        "ollama": {
            ...
            "assistant_model": settings.assistant_llm_model,
            "assistant_model_available": True,
        },
    }
