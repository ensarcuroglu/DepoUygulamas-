"""Faz 0 healthz and internal-auth tests."""
...
def mock_ollama_ok(monkeypatch: pytest.MonkeyPatch):
    async def fake_fetch(settings):
        return {"models": [{"name": settings.ollama_text_model}]}
...
def test_healthz_valid_key_returns_200(mock_ollama_ok):
    ...
    assert body["service"] == "DocAiService"
    assert body["version"] == "0.1.0"
    assert body["ollama"]["text_model"] == TEST_MODEL
    assert body["ollama"]["text_model_available"] is True
...
def test_healthz_missing_text_model_returns_503(monkeypatch: pytest.MonkeyPatch):
    async def fake_fetch(settings):
    ...
    assert response.status_code == 503
    assert response.json()["detail"] == f"Ollama text model bulunamadi: {TEST_MODEL}"
